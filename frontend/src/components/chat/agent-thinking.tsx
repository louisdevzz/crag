...
/**
 * Agent Thinking — a compact "thinking" indicator that sits above the chat
 * composer while a response is being worked on. Adapted from boardui's
 * Agent Thinking component: https://www.boardui.com/components/agent-thinking
 *
 * Four variants:
 ...
 *
 * Pairs the indicator with a shimmering label and an elapsed timer that
 * starts when the loader mounts.
 */
...
function ElapsedTimer() {
  ...
  return <span className="font-mono text-[11px] tabular-nums text-muted-foreground">{elapsed.toFixed(1)}s</span>;
}
...
export function AgentThinking({
  ...
  return (
    ...
    >
      ...
      <span aria-label={label} className={cn("text-xs font-medium", shimmer && "bui-agent-thinking-label")}>
        {label}
      </span>
