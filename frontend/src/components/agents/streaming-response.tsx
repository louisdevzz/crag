...
function ResponseAction({
  ...
  return (
    <motion.button
      ...
      aria-label={label}
      title={label}
      aria-pressed={label === "Hữu ích" || label === "Chưa hữu ích" ? active : undefined}
      onClick={onClick}
      whileTap={reduce ? undefined : { scale: 0.9 }}
      transition={SPRING_PRESS}
      className={cn(
        "grid size-7 place-items-center rounded-md text-muted-foreground outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-1 focus-visible:ring-ring cursor-pointer",
  ...
  return (
    ...
    >
      <div
        ...
        className={cn("text-sm leading-6 text-foreground", contentClassName)}
      >
        {children}
      </div>
        ...
        {shouldShowActions ? (
          ...
          >
            <div className={cn("flex items-center gap-1", actionsClassName)}>
              {canCopy ? (
                <ResponseAction
                  label={copied ? "Đã sao chép" : "Sao chép câu trả lời"}
                  onClick={handleCopy}
                >
                  {copied ? (
              ...
              {onRetry ? (
                <ResponseAction label="Tạo lại câu trả lời" onClick={onRetry}>
              ...
              {hasSources ? (
                <button
                  ...
                  className="group ml-auto inline-flex min-h-7 items-center gap-2 rounded-lg px-2 text-xs text-muted-foreground outline-none transition-colors hover:bg-muted/50 hover:text-foreground focus-visible:ring-1 focus-visible:ring-ring cursor-pointer"
                >
                  <CitationStack citations={sources} />
                  <span className="tabular-nums font-medium">
                    {sources.length} căn cứ trích dẫn
                  </span>
                  <motion.span
                    aria-hidden="true"
