...
export default function LoadingState({
  ...
  const internalElapsed = useElapsed();
  ...
  return (
    <div
      role="status"
      className={`flex w-fit items-center gap-2.5 rounded-control bg-surface/80 px-2.5 py-1.5 shadow-hairline backdrop-blur-xs ${className}`}
      style={{ animation: "fade-in 200ms ease-out both" }}
    >
      <LoaderGrid delays={delays} dur={dur} round={round} />
      <span
        className="bg-clip-text text-[13px] font-medium text-transparent select-none"
        style={{
          backgroundImage:
